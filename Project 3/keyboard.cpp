...
//checks if character is a letter A-G
bool isLetter(char letter) {
...
//checks if character is accidental # or b
bool isAccidental(char acc) {
    if (acc == 'b' || acc == '#')
        return true;
    return false;
...
//checks if string is a note:
// letter
// letter + accidental
// letter + digit
// letter + accidental + digit
bool isNote(string note){
    if (note == "")
        return true;
    int len = note.length();
    if (!isLetter(note[0])) {   // first character must be a letter
        return false;
    }
    for (int i = 1; i < len; i++){
        if (isAccidental(note[i])) {   // if char is accidental, the previous char must be a letter
            if(!isLetter(note[i-1])){
                return false;
            }
        }
        else if (isdigit(note[i])) {    // if char is digit, the previous char must be a letter or accidental
...
// returns true if parameter is a song string
// song string: does not contain any notes that are not playable
bool hasCorrectSyntax(string song){
    if (song == "")
        return true;
    int len = song.length();
    if (song[len-1] != '/')     // must end with a /
        return false;
    string chord = "";
    for (int i = 0; i < len; i++) {
        if (song[i] == '/') {       // break string into chords
            if (!isNote(chord))     // check if chord is a note
                return false;
            else {
                chord = "";
                continue;
...
bool isPlayableNote(string note) {
    ...
    for (int i=0; i < len; i++) {
        if (note[i] == '0' || note[i] == '7' || note[i] == '8' || note[i] == '9')
            return false;
        if (note[i] == '1') {
            if (note[i-1] != '#')   // B#1 is playable
                return false;
        }
        if (note[i] == '6') {
            if (note[i-1] != 'b' && note[i-1] != 'C')   // Cb6 is playable
                return false;
        }
        if (note[i] == '2') {
            if (note[i-1] == 'b') {         // Cb2 is not a playable note
                if (note[i-2] == 'C')
                    return false;
            }
        }
...
bool isPlayable(string song){
    if (song == "")
        return true;
    int len = song.length();
    if (song[len-1] != '/')     // song must end in /
        return false;
    string chord = "";
    for (int i = 0; i < len; i++) {
        if (song[i] == '/') {       // splits song into chords
            chord += song[i];
            if (!isPlayableNote(chord))     // checks that each chord is a playable note
                return false;
            else {
                chord = "";
                continue;
...
// if parameter song is a playable song, set instructions to encoding of song, leave badBeat unchanged, return 0
// if parameter song is not playable, leave instructions unchanged
// if parameter song is not a song string, leave badBeat unchanged and return 1
// if parameter song is a song string but is not playable, set badBeat to the number of the beat with the first unplayable note, return 2
int encodeSong(string song, string& instructions, int& badBeat){
    if(isPlayable(song)) {
        string encoding = "";
        int len = song.length();
        ...
        if (countSlash == len) {            // if the entire string is slashes
            for (int i = 0; i < len; i++) {
                encoding += " ";
            }
        ...
        for (int i = 0; i < song.length(); i++){
            ...
            if (song[i] != '/') {
                chord += song[i];       // get all the notes in the chord
            }
            else {
                int chordLen = chord.length();
                if (chordLen == 0) {            // just a slash
                    encoding += " ";
                    song = song.substr(1, song.length()-1);
                    chord = "";
                    i = -1;
                }
                else if (chordLen == 1) {       // just a letter
                    encoding += encodeNote(4, chord[0], ' ');
                    song = song.substr(2, song.length()-2);     // get remainder of chord
                    chord = "";
                    i = -1;
                }
                else if (chordLen == 2 && (isdigit(chord[1]) || isAccidental(chord[1]))) {  // letter + digit OR letter + accidental
                    if (isAccidental(chord[1])) {       // letter + accidental
                        encoding += encodeNote(4, chord[0], chord[1]);
                        song = song.substr(3, song.length()-3);     // get the remainder
                        chord = "";                                 // reset chord
                        i = -1;
                    }
                    if (isdigit(chord[1])) {        // letter + digit
                        int num = chord[1] - 48;
                        encoding += encodeNote(num, chord[0], ' ');
                        song = song.substr(3, song.length()-3);     // get remainder
                        chord = "";
                        i = -1;
                    }
                }
                else if (chordLen == 3 && isdigit(chord[2]) && isAccidental(chord[1])) {        // letter + accidental + digit
                    int num = chord[2] - 48;
                    ...
                    while (chord.length() > 1) {
                        ...
                        if (isdigit(chord[1])) {        // if chord is letter + digit
                            int num = chord[1] - 48;
                            encoding += encodeNote(num, chord[0], ' ');
                            chord = chord.substr(2, cLen-2);    // remove the part that was just encoded
                        }
                        else if (isAccidental(chord[1])) {
                            if(isdigit(chord[2])) {             // if chord is letter + accidental + digit
                                int num = chord[2] - 48;
                                encoding += encodeNote(num, chord[0], chord[1]);
                                chord = chord.substr(3, cLen-3);
                            }
                            else {                  // if chord is letter + accidental
                                encoding += encodeNote(4, chord[0], chord[1]);
                    ...
                    if (chord.length() == 1)    // just a letter
                        encoding += encodeNote(4, chord[0], ' ');
                    encoding += ']';
    ...
    if(!hasCorrectSyntax(song))     // not song string
        return 1;
    ...
    for (int i = 0; i < len; i++) {
        if (song[i] == '/') {
            chord += song[i];
            if (isPlayable(chord))      // find the badBeat
                beatCount++;
            else {
                badBeat = beatCount + 1;
                return 2;
